<?php

use App\Actions\Eod\EodPipelinePayload;
use App\Actions\Eod\Steps\AccrueDepositInterest;
use App\Enums\DepositStatus;
use App\Enums\EodStatus;
use App\Models\Branch;
use App\Models\Customer;
use App\Models\DepositAccount;
use App\Models\DepositProduct;
use App\Models\DepositProductRate;
use App\Models\EodProcess;
use App\Models\EodProcessStep;
use App\Models\User;
use Carbon\Carbon;

describe('AccrueDepositInterest', function (): void {
    beforeEach(function (): void {
        $this->branch = Branch::create([
            'code' => '001',
            'name' => 'Cabang Utama',
            'is_head_office' => true,
            'is_active' => true,
        ]);

        $this->user = User::factory()->create(['branch_id' => $this->branch->id]);

        $this->process = EodProcess::create([
            'process_date' => now()->toDateString(),
            'status' => EodStatus::Running,
            'total_steps' => 11,
            'completed_steps' => 0,
            'started_at' => now(),
            'started_by' => $this->user->id,
        ]);

        $this->payload = new EodPipelinePayload(
            process: $this->process,
            processDate: Carbon::parse('2026-03-10'),
            performer: $this->user,
        );

        $this->next = fn (EodPipelinePayload $p) => $p;
    });

    it('skips execution when step is already completed', function (): void {
        EodProcessStep::create([
            'eod_process_id' => $this->process->id,
            'step_number' => AccrueDepositInterest::STEP_NUMBER,
            'step_name' => AccrueDepositInterest::STEP_NAME,
            'status' => EodStatus::Completed,
        ]);

        $step = app(AccrueDepositInterest::class);
        $result = $step->handle($this->payload, $this->next);

        expect($result)->toBeInstanceOf(EodPipelinePayload::class)
            ->and($this->process->fresh()->completed_steps)->toBe(0);
    });

    it('accrues interest for all active deposit accounts', function (): void {
        $product = DepositProduct::factory()->create(['code' => 'DEP']);

        DepositProductRate::create([
            'deposit_product_id' => $product->id,
            'tenor_months' => 12,
            'min_amount' => 1000000,
            'max_amount' => null,
            'interest_rate' => 6.0,
            'is_active' => true,
        ]);

        $customer = Customer::factory()->create([
            'branch_id' => $this->branch->id,
            'created_by' => $this->user->id,
            'approved_by' => $this->user->id,
        ]);

        $account = DepositAccount::factory()->create([
            'customer_id' => $customer->id,
            'deposit_product_id' => $product->id,
            'branch_id' => $this->branch->id,
            'status' => DepositStatus::Active,
            'principal_amount' => 10000000,
            'interest_rate' => 6.0,
        ]);

        $step = app(AccrueDepositInterest::class);
        $step->handle($this->payload, $this->next);

        $processStep = $this->process->steps()->where('step_number', AccrueDepositInterest::STEP_NUMBER)->first();
        expect($processStep->status)->toBe(EodStatus::Completed)
            ->and($processStep->records_processed)->toBe(1)
            ->and($this->process->fresh()->completed_steps)->toBe(1);

        $account->refresh();
        expect((float) $account->accrued_interest)->toBeGreaterThan(0);
    });

    it('processes zero accounts when none are active', function (): void {
        $step = app(AccrueDepositInterest::class);
        $step->handle($this->payload, $this->next);

        $processStep = $this->process->steps()->where('step_number', AccrueDepositInterest::STEP_NUMBER)->first();
        expect($processStep->status)->toBe(EodStatus::Completed)
            ->and($processStep->records_processed)->toBe(0)
            ->and($this->process->fresh()->completed_steps)->toBe(1);
    });

    it('ignores matured deposit accounts', function (): void {
        $product = DepositProduct::factory()->create(['code' => 'DEP']);

        $customer = Customer::factory()->create([
            'branch_id' => $this->branch->id,
            'created_by' => $this->user->id,
            'approved_by' => $this->user->id,
        ]);

        DepositAccount::factory()->create([
            'customer_id' => $customer->id,
            'deposit_product_id' => $product->id,
            'branch_id' => $this->branch->id,
            'status' => DepositStatus::Matured,
            'principal_amount' => 10000000,
            'interest_rate' => 6.0,
        ]);

        $step = app(AccrueDepositInterest::class);
        $step->handle($this->payload, $this->next);

        $processStep = $this->process->steps()->where('step_number', AccrueDepositInterest::STEP_NUMBER)->first();
        expect($processStep->records_processed)->toBe(0);
    });

    it('creates step record if not exists', function (): void {
        expect($this->process->steps()->count())->toBe(0);

        $step = app(AccrueDepositInterest::class);
        $step->handle($this->payload, $this->next);

        $processStep = $this->process->steps()->where('step_number', AccrueDepositInterest::STEP_NUMBER)->first();
        expect($processStep)->not->toBeNull()
            ->and($processStep->step_name)->toBe('Deposit Interest Accrual');
    });
});
